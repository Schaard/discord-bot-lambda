...
class DynamoDBHandler:
    ...
    #Updating DB
    def add_kill(self, submitter_id, user_id, target_user_id, cause_of_death, server_id, game_id, channel_id, timestamp, unforgivable=False, forgiven=False, evidence_link = None):
        try:
            kill_record = {
    ...
    def get_grudge_string(self, user_id, user_kills, compare_user, compare_kills, no_article = False):
        ...
        lead_descriptors = {
            ...
            15: "🏰 GRUDGEHOLDE: an stronghold built of pure grudge 🏰",
    ...
    def _build_report(self, kill_stats, processed_items, start_date, end_date, active_entitlement = False):
        ...
        # Create the embed
        embed = discord.Embed(
            title=f"🌟 Your Server's Month in Grudges ({month_year}) 🌟",
        ...
        if kill_stats['kills_by_channel']:
            ...
            embed.add_field(name="🏞️ Peaceful Channels 🏞️", 
                            value="All channels seem peaceful. No danger zones detected!", 
                            inline=False)

        if total_kills == 0:
            embed.add_field(name="☮️ Peace Reigns ☮️", value="No one's racking up kills...yet!", inline=False)
        else:
            # Top Killer
            ...
            if most_forgiven:
                ...
                embed.add_field(name="🕊️ Unforgiving Server 🕊️", value="No one's been forgiven...yet!", inline=False)
            ...
            if kill_stats['last_incident']:
                ...
                if active_entitlement:
                    highlight_title = "👁️ GrudgeKeeper Premium Benefit 👁️"
                    highlight_message = "Thanks to the server's GrudgeKeeper Premium subscription, all these grudges carry on."
                else:
                    highlight_title = "🔄 Grudges Have Been Reset 🔄"
                    highlight_message = "GrudgeKeeper Free resets all grudges monthly. Upgrade to GrudgeKeeper Premium to hold grudges forever!"
        ...
        try:
            ...
            def process_kill(kill, killer_id, target_id):
                ...
                if server_id is None or server_value == server_id:
                    ...
                    kill_date = datetime.fromisoformat(timestamp)
                    
                    if active_entitlement or (kill_date.year == current_date.year and kill_date.month == current_date.month):
                        incident = {
                            'UserId': killer_id,
                            ...
                            'EvidenceLink': kill.get('EvidenceLink', ''),
                        }
                    else:
                        incident = {
                            'UserId': '***',
            ...
            for incident in paginated_incidents:
                ...
                evidence_link = incident.get('EvidenceLink', 'None')
                ...
                if cause_of_death and cause_of_death.lower() != "unknown":
                    cod_and_lw_combostring += f"Killed by {cause_of_death.strip()}.\n"
                if last_words and last_words != "None":
                    cod_and_lw_combostring += f"\n{last_words.strip()}"
                if evidence_link and evidence_link != "None": 
                    cod_and_lw_combostring += f"\nLink: {evidence_link}"
                if cod_and_lw_combostring != "": embed.add_field(name="Notes", value=f"{cod_and_lw_combostring}", inline=True)
